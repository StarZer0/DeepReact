(function (global, factory) {
  ...
  function invokeGuardedCallbackAndCatchFirstError(name, func, context, a) {
  ...
  function isKeypressCommand(nativeEvent) {
    return (
      (nativeEvent.ctrlKey || nativeEvent.altKey || nativeEvent.metaKey) && !(nativeEvent.ctrlKey && nativeEvent.altKey)
    );
  }
  /**
  ...
  function extractEvents$1(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget) {
  ...
  function extractEvents$2(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags) {
  ...
  function dispatchEventsForPlugins(domEventName, eventSystemFlags, nativeEvent, targetInst) {
    var nativeEventTarget = getEventTarget(nativeEvent);
    var dispatchQueue = [];
  ...
  function createInstance(type, props, rootContainerInstance, hostContext, internalInstanceHandle) {
    var parentNamespace = hostContext;
  ...
  /** 创建update对象(环形链表) 1. 应用初始化 2. 发起组件更新 */
  ...
  /** 将update放到fiber.shared.pending上 */
  ...
  function ChildReconciler(shouldTrackSideEffects) {
    ...
    function reconcileChildrenArray(returnFiber, currentFirstChild, newChildren, lanes) {
      ...
      if (oldFiber === null) {
        for (; newIdx < newChildren.length; newIdx++) {
          ...
          // 更新创建的fiber节点的index位置
          lastPlacedIndex = placeChild(_newFiber, lastPlacedIndex, newIdx);

          // 构建创建的子fiber节点间的关系, 并确保最终返回第一个子fiber节点
          if (previousNewFiber === null) {
     ...
     * @param {FiberNode | null} currentFirstChild 当前渲染的fiber节点
     * @param {import("react").ReactElement} element 新节点对应的ReactElement
     * @param {number} lanes
     * @returns
     */
    ...
    /**
     *
     * @param {FiberNode} returnFiber 父节点fiber
     * @param {FiberNode | null} currentFirstChild
     * @param {import("react").ReactElement} newChild
     * @param {number} lanes
     * @returns
     */
  ...
  function cloneChildFibers(current, workInProgress) {
    ...
    //
    // 因为涉及到fiber节点的复用, 清空同层级最后一个fiber的sibling
  ...
  function renderWithHooks(current, workInProgress, Component, props, secondArg, nextRenderLanes) {
    ...
    // 全局标记当前render的fiber节点,用于hook的dispatch绑定关联的fiber对象(通过.bind的参数进行关联)
    ...
    // 更新dispatcher对象, 根据fiber的状态决定使用不同的hook(mount或update),
    // 之后执行Component方法时, 取到的useState等hook会根据Dispatcher的不同在不同阶段取得不同的值
    ReactCurrentDispatcher$1.current =
      current === null || current.memoizedState === null ? HooksDispatcherOnMount : HooksDispatcherOnUpdate;

    // 执行一次函数组件, 并取得当前的ReactElement树
    var children = Component(props, secondArg);

    // 只有在render过程中通过dispatchAction触发更新才会进入该流程
  ...
  function mountState(initialState) {
    ...
    // 缓存初始值, 并将初始值赋值给memoizedState用于记录
    hook.memoizedState = hook.baseState = initialState;
    // 创建一个用于管理state更新的队列, 并初始化相关属性
    var queue = (hook.queue = {
      pending: null,
      dispatch: null,
      lastRenderedReducer: basicStateReducer,
  ...
  function dispatchAction(fiber, queue, action) {
    ...
    // 判断是否在当前渲染过程中触发了state更新
    if (fiber === currentlyRenderingFiber$1 || (alternate !== null && alternate === currentlyRenderingFiber$1)) {
      didScheduleRenderPhaseUpdateDuringThisPass = didScheduleRenderPhaseUpdate = true;
    } else {
      // 如果不是在当前渲染过程中触发的state更新, 需要进一步处理
      if (fiber.lanes === NoLanes && (alternate === null || alternate.lanes === NoLanes)) {
        //  如果待更新的 Fiber 节点的优先级为0（表示当前没有更高优先级的更新），且它的 alternate 节点也没有优先级
        ...
        if (lastRenderedReducer !== null) {
          try {
            ...
            // 记录优化后的 reducer 和对应的 state
            update.eagerReducer = lastRenderedReducer;
            update.eagerState = eagerState;

            // 如果优化后的 state 与当前 state 相同，则不执行后续更新操作
  ...
  /** 根据节点前后ref情况判断是否需要添加Ref标记 */
  function markRef(current, workInProgress) {
    var ref = workInProgress.ref;

    // mount节点时, 或者更新前后ref发生变更, 添加Ref标记
  ...
  function updateFunctionComponent(current, workInProgress, Component, nextProps, renderLanes) {
    ...
    // 调用renderHooks处理函数组件, 获取函数组件返回值children
    nextChildren = renderWithHooks(current, workInProgress, Component, nextProps, context, renderLanes);

    // beginWork中检测到组件不需要更新, bailout跳出当前函数组件fiber的执行
    if (current !== null && !didReceiveUpdate) {
      bailoutHooks(current, workInProgress, renderLanes);
      return bailoutOnAlreadyFinishedWork(current, workInProgress, renderLanes);
    }
  ...
  function updateHostComponent(current, workInProgress, renderLanes) {
    // 添加host context: HTML 或 SVG 或 Math
    ...
    var nextChildren = nextProps.children;
    // 是否是文本节点 textarea、option、dangerouslySetInnerHTML
    var isDirectTextChild = shouldSetTextContent(type, nextProps);

    if (isDirectTextChild) {
      // 如果是文本节点, 可以直接忽略后续的fiber创建
      nextChildren = null;
    } else if (prevProps !== null && shouldSetTextContent(type, prevProps)) {
      // 如果之前是文本节点, 现在不是, 添加ContentReset标记
  ...
  /** 用于创建一个不确定类型的组件(组件动态变化、组件异步加载) */
  function mountIndeterminateComponent(_current, workInProgress, Component, renderLanes) {
    // 如果当前组件存在, 意味着这可能是一次重新渲染, 可能是由于suspend引起的
    // 重置alternate链并在workInProgress fiber上设置Placement状态
    if (_current !== null) {
      _current.alternate = null;
      workInProgress.alternate = null;
    ...
    // 获取未屏蔽的context并创建组件的屏蔽context
    var unmaskedContext = getUnmaskedContext(workInProgress, Component, false);
    var context = getMaskedContext(workInProgress, unmaskedContext);

    // 准备读取context
    prepareToReadContext(workInProgress, renderLanes);
    // 调用函数组件, 并获取组件返回的children
    ...
    if (
      ...
      // 通过判断render函数存在并且不包含$$typeof字段来判断是class组件, 并执行相应的初始化流程
  ...
  function shouldRemainOnFallback(suspenseContext, current) {
    if (current !== null) {
  ...
  function bailoutOnAlreadyFinishedWork(current, workInProgress, renderLanes) {
    ...
    // ⭐: scheduleUpdateOnFiber调度更新时会将更新fiber的lane合并到所有祖先节点的childLanes中
    // 判断renderLanes是否包含更新lane来决定是否跳过当前workInProgress节点的子节点beginWork等处理
    if (!includesSomeLane(renderLanes, workInProgress.childLanes)) {
      return null;
    } else {
      // 无法跳过子节点, 说明当前fiber树分支存在待更新节点
   ...
   * @param {FiberNode | null} workInProgress
   * @param {number} renderLanes
   * @return {FiberNode | null} 子节点
   */
  function beginWork(current, workInProgress, renderLanes) {
    ...
    if (current !== null) {
      ...
      if (oldProps !== newProps || hasContextChanged()) {
        ...
        // 当前fiber节点没有更新 (renderLanes不包含当前fiber.lanes)
        didReceiveUpdate = false;
      ...
      // 节点mount阶段: 第一次mount时, 节点没有对应的fiber存在
      didReceiveUpdate = false;
    }
    workInProgress.lanes = NoLanes;
    ...
    switch (workInProgress.tag) {
      /**
       * 函数组件第一次渲染时其tag会被设置成IndeterminateComponent来表达其不确定性
       * 1. 异步分片渲染下, 存在“最优先渲染”节点处理优先级最高的任务
       *    如果在该节点遇到函数组件, react需要快速创建对应的fiber节点来表示该组件, 但是由于组件执行结果未知, 故设置tag为IndeterminateComponent
       *    这样可以继续进行后续优先级计算而无需等待函数组件的执行结果
       */
      case IndeterminateComponent: {
  ...
  var appendAllChildren = function (parent, workInProgress) {
  ...
  var updateHostContainer = function () {};

  var updateHostComponent$1 = function (current, workInProgress, type, newProps) {
    var oldProps = current.memoizedProps;
  ...
  function schedulePassiveEffects(finishedWork) {
    ...
    if (lastEffect !== null) {
      ...
      // 循环effect链表, 将unmount, mount添加进effect队列
      do {
        var _effect = effect,
          next = _effect.next,
          tag = _effect.tag;
  ...
  function commitPlacement(finishedWork) {
    ...
    switch (parentFiber.tag) {
      ...
      // HostRoot和HostPortal都是容器节点, 和HostComponent的区别是这两者可能是注释节点需要单独处理
      // HostComponent必不可能是注释节点(HostComponent是由ReactElement转化而来, 如果是注释节点, 类型不是HostComponent)
      case HostRoot:
  ...
  /**
   * 在fiber节点上开始调度update更新
   * @param {*} fiber 需要更新的fiber节点
   * @param {*} lane update的优先级
   * @param {*} eventTime update的发起时间
   * @returns
   */
  function scheduleUpdateOnFiber(fiber, lane, eventTime) {
    checkForNestedUpdates();
    // ⭐: 主要作用于对比更新阶段
    // 会自下而上将当前调度的fiber的lane合并到所有祖先节点上, 对比更新时会根据lanes来决定是否复用节点
    ...
    if (lane === SyncLane) {
      ...
      if (
        ...
        if (executionContext === NoContext) {
          resetRenderTimer();
          // 取消schedule调度, 主动刷新回调队列并立即进入fiber树构造过程
  ...
  function markUpdateLaneFromFiberToRoot(sourceFiber, lane) {
    console.log("[MarkUpdateLaneFromFiberToRoot]:", "自下而上合并lanes, 更新fiber.childLanes");
    // 将lane合并到当前调度的fiber上
    sourceFiber.lanes = mergeLanes(sourceFiber.lanes, lane);
    var alternate = sourceFiber.alternate;

    // ??? 为什么要将lane合并到当前展示的fiber上
    if (alternate !== null) {
    ...
    // 递归向上, 将当前需要更新的lane合并进所有祖先节点的childLanes中
    // ⭐: 主要用于bailout阶段决定是否要处理其子节点 -> 比如同级AB节点, A节点下有更新, B下无更新, A会包含子节点更新Lane, B在执行bailout时不包含更新, 会跳过子节点的beginWork
    while (parent !== null) {
  ...
  function ensureRootIsScheduled(root, currentTime) {
    ...
    // 如果没有更新任务(空Lane) 中断上一帧执行的任务,并重置状态并返回
    if (nextLanes === NoLanes) {
    ...
    if (existingCallbackNode !== null) {
      ...
      // 未完成的协调任务和当前最新的任务优先级一致, 退出最新的任务执行, 继续之前的任务
    ...
    if (newCallbackPriority === SyncLanePriority) {
      ...
      console.log("schedule new callback");
  ...
  function performConcurrentWorkOnRoot(root) {
    ...
    // 判断当前帧执行完调度后任务是否发生改变, ensureRootIsScheduled用于终止、更新调度任务
    // 如果任务没有变化, 返回当前函数, Scheudler模块会在下一帧继续执行当前函数
    if (root.callbackNode === originalCallbackNode) {
  ...
  function performSyncWorkOnRoot(root) {
    ...
    if (root === workInProgressRoot && includesSomeLane(root.expiredLanes, workInProgressRootRenderLanes)) {
      // 初次构造时root = fiberRoot, workInProgressRoot = null, 因此只有对比更新才会进入该分支
  ...
  function performUnitOfWork(unitOfWork) {
    ...
    // 构建fiber, 并返回第一个child节点
    next = beginWork(current, unitOfWork, subtreeRenderLanes);

    // 执行前pendingPorps包含了需要更新的内容, 其中也包含了children属性
    // 执行完构建后, 代表当前的pendingProps已处理, 存到memoizedProps缓存
    unitOfWork.memoizedProps = unitOfWork.pendingProps;

    // 如果不存在子节点, 调用completedUnitOfWork完成当前fiber节点的构建
    // 否则将workInProgress指向子节点并开始子节点的beginWork
    // 这也表明了react遵循的深度优先遍历, 如果对当前节点调用completeWork则表示其子节点也处理完成了
    if (next === null) {
  ...
  function completeUnitOfWork(unitOfWork) {
    ...
    /** 循环从当前完成的fiber节点开始递归向上, 处理节点的effect列表 */
    do {
      ...
      if ((completedWork.flags & Incomplete) === NoFlags) {
        ...
        // Incomplete在throwException函数中赋值给fiber.flags returnFiber.flags & Incomplete === NoFlags 代表returnFiber.flags不包含Incomplete
        // 判断是否存在父节点且父节点的执行未被中断
        if (returnFiber !== null && (returnFiber.flags & Incomplete) === NoFlags) {
          /** 下面这一块的逻辑: 将子节点的effect list 添加到父节点的effect list之后 */
          // firstEffect默认指向effect链第一个fiber节点
          // 将父节点的第一个effect节点指向当前fiber得到的firstEffect, 这里是因为当前completedWork之前可能存在sibling节点
          if (returnFiber.firstEffect === null) {
            returnFiber.firstEffect = completedWork.firstEffect;
          }

          // 因为fiber的begin和complete过程是一个深度优先遍历, completedWork.lastEffect 存在, 表明当前fiber节点的子节点存在具有effect的fiber节点
          if (completedWork.lastEffect !== null) {
            // 更新当前节点的父节点的lastEffect的下一个effect节点为当前节点下的第一个具有effect的节点
            if (returnFiber.lastEffect !== null) {
              returnFiber.lastEffect.nextEffect = completedWork.firstEffect;
            }

            // 保持更新父节点的lastEffect为当前所有具有effect的fiber节点的最后一个节点
            returnFiber.lastEffect = completedWork.lastEffect;
          }

          var flags = completedWork.flags;

          /** 更新副作用链: react通过lastEffect, nextEffect, firstEffect来链接具有更新的fiber节点, 从而跳过不需要更新的节点 */
          // 判断flag是否存在更新 PerformedWork标记用于调试, 任何大于PerformedWork的标记代表存在更新
          if (flags > PerformedWork) {
            if (returnFiber.lastEffect !== null) {
              // 将当前fiber节点添加到上一次completed的fiber的nextEffect链中
              returnFiber.lastEffect.nextEffect = completedWork;
            } else {
              // 父节点不存在具有更新的节点, 直接修改firstEffect为当前fiber, 表示当前节点是第一个具有更新的节点
              returnFiber.firstEffect = completedWork;
            }

            // 父节点的最后一个effect节点变更为当前节点, 完成effect链的变更
  ...
  function commitRootImpl(root, renderPriorityLevel) {
    ...
    // root.finishedWork默认指向当前workInProgress fiber树的根节点(root.current.alternate)
    var finishedWork = root.finishedWork;
    // root.finishedLanes是本次render的lanes
  ...
  function commitBeforeMutationEffects() {
    while (nextEffect !== null) {
      ...
      // 针对Snapshot标记的执行相应的生命周期函数(主要针对class组件的getSnapshotBeforeUpdate生命周期函数)
  ...
  function flushPassiveEffectsImpl() {
    ...
    // 获取待处理的unmount passive effects队列
    // unmountEffects只会存在更新阶段, 在第一次render的commit后才会存在unmountHookEffects
    var unmountEffects = pendingPassiveHookEffectsUnmount;
    pendingPassiveHookEffectsUnmount = [];

    // 执行所有的unmount hook effects
    ...
    // 从当前展示的fiber树根节点开始遍历并移除具有更新的fiber节点之间的关系
    var effect = root.current.firstEffect;
    while (effect !== null) {
      var nextNextEffect = effect.nextEffect;
      effect.nextEffect = null;

      // 节点需要被删除, 删除节点相关索引, 避免内存泄漏
  ...
  function FiberNode(tag, pendingProps, key, mode) {
    ...
    this.memoizedProps = null; // 存储组件的属性: 父组件传递的值
    this.updateQueue = null;
    this.memoizedState = null; // 存储组件的状态: 组件状态的变化会导致组件被重新渲染
    this.dependencies = null;
    this.mode = mode;

    /** 标志位: reconciler阶段会将所有拥有flags标记的节点添加到副作用链表中 */
    this.flags = NoFlags;
    /** 单向链表: 指向下一个具有effect的fiber节点 */
...
function logStacks(message) {
  try {
    throw new Error(message || "Log Stack");
  } catch ({ stack }) {
    stack = stack.replace(/^Error:\s/, "[Stacks]: ");
    stack = stack.replace(/\s+at\slogStacks[^\n]*\n/, "\n");
    console.log(stack);
  }
}
