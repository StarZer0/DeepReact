...
const enableProfilerNestedUpdateScheduledHook =
...
const Hydrating =
  /*                    */
  0b00000000000000100000000000; // You can change the rest (and add more).
...
const HostEffectMask =
  /*               */
  0b00000000000011111111111111; // These are not really side effects, but we still reuse this field.
...
const Forked =
  /*                       */
  0b00000010000000000000000000; // Static tags describe aspects of a fiber that are not specific to a render,
...
const PassiveStatic =
  /*                */
  0b00010000000000000000000000; // Flag used to identify newly inserted fibers. It isn't reset after commit unlike `Placement`.
...
function getEventKey(nativeEvent) {
  if (nativeEvent.key) {
    ...
    const key = normalizeKey[nativeEvent.key] || nativeEvent.key; // $FlowFixMe unable to index with a `mixed` value
...
function getResource(type, pendingProps, currentProps) {
  ...
  switch (type) {
    case "base": {
      ...
      const target = pendingProps.target,
    ...
    case "meta": {
      ...
      const charSet = pendingProps.charSet,
      ...
      const _getResourcesFromRoot = getResourcesFromRoot(headRoot),
    ...
    case "link": {
      const rel = pendingProps.rel;

      switch (rel) {
        case "stylesheet": {
          const styleResources = getResourcesFromRoot(resourceRoot).styles;

          const precedence = pendingProps.precedence,
            href = pendingProps.href;
  ...
  if (!instance) {
    ...
    switch (type) {
      ...
      case "meta": {
        ...
        const matcher = metaResource.matcher,
      ...
      case "base": {
        ...
        const matcher = baseResource.matcher;
...
function acquireStyleResource(resource) {
  ...
  if (!instance) {
    ...
    const existingEl = root.querySelector(
      'link[rel="stylesheet"][data-precedence][href="' +
        limitedEscapedHref +
        '"]'
    );
...
function insertStyleInstance(instance, precedence, root) {
  ...
  if (prior) {
    ...
    if (parent) {
      ...
      throw Error(formatProdErrorMessage(447));
    }
  }
}
...
const TransitionRoot = 0;
const TransitionTracingMarker = 1;
  ...
  if (enableTransitionTracing) {
    if (pendingTransitions !== null) {
      ...
      if (onMarkerIncomplete != null && markerIncomplete !== null) {
        markerIncomplete.forEach((_ref, markerName) => {
          let transitions = _ref.transitions,
            aborts = _ref.aborts;
          transitions.forEach(transition => {
...
function getRetryCache(finishedWork) {
  ...
  switch (finishedWork.tag) {
    ...
    case OffscreenComponent: {
      ...
      let retryCache = instance._retryCache; // $FlowFixMe[incompatible-type] found when upgrading Flow
...
function commitMutationEffectsOnFiber(finishedWork, root, lanes) {
  ...
  switch (finishedWork.tag) {
    ...
    case OffscreenComponent: {
      ...
      if (flags & Visibility) {
        const offscreenInstance = finishedWork.stateNode; // Track the current state on the Offscreen instance so we can
...
const ReactCurrentActQueue$1 = ReactSharedInternals.ReactCurrentActQueue;
...
const SuspendedAndReadyToUnwind = 5; // When this is true, the work-in-progress fiber just suspended (or errored) and
    ...
    if (prevRootTransitionCallbacks !== null) {
      schedulePostPaintCallback(endTime => {
        ...
        if (prevPendingTransitionCallbacks !== null) {
          ...
          scheduleCallback$2(IdlePriority, () => {
...
function preinit$1() {
  const dispatcher = Internals.Dispatcher.current;
