...
// Lane用来区分任务的优先级，比Scheduler更加细粒度
// 1. 每个lane优先级都是一个31位二进制数字, 1表示该位置可用, 2表示该位置不可用
// 2. 通过二进制按位与来判断a, b代表的lane是否存在交际

/**
 * Lane(赛道)用来区分任务优先级，有比scheduler更细粒度的区分
 * 1. 一个lane用31位二进制来表示31条赛道，位数越小的赛道优先级越高, 某些相邻的赛道具有相同优先级
 *    1 表示该赛道被使用 0表示该赛道未使用
 *    其中，同步优先级占用的赛道位第一位: SyncLane
 * 2. “批”的概念: TransitionLanes 和 RetryLanes 占用了几条赛道，表示拥有的优先级范围
 *    越低优先级的lanes占用位越多，比如SyncLane只占用了一个，优先级最高，TransitionLanes占用了9位，优先级很低
 *    优先级越低的更新越容易被打断,导致积压下来
 * 3. 优先级的计算直接采用位运算，效率更高
 */
 ...
 * 获取下一个最高优先级的lanes
 * @param {*} root
 * @param {*} wipLanes 正在执行任务的lane
 * @returns
 */
export function getNextLanes(root: FiberRoot, wipLanes: Lanes): Lanes {
  // Early bailout if there's no pending work left.
  // pendingLanes保存了所有将要执行的任务的lane
  // 如果pendingLanes为空，表示任务全部执行完成, 也就不需要更新了, 直接跳出
  ...
  if (nonIdlePendingLanes !== NoLanes) {
    // 从非闲置lanes中移除挂起的任务suspendedLanes
    const nonIdleUnblockedLanes = nonIdlePendingLanes & ~suspendedLanes;
    if (nonIdleUnblockedLanes !== NoLanes) {
      // 从剩下未被阻塞的任务中找出优先级最高的任务执行
      nextLanes = getHighestPriorityLanes(nonIdleUnblockedLanes);
    } else {
      // 没有未被阻塞的任务，直接从挂起的任务中找优先级最高的执行
  ...
  // higher priority.
  // 如果我们已经渲染到一半，切换lanes会终止它并且我们会丢失进度。我们应当只有在新lanes优先级更高时才这么做
  if (
    wipLanes !== NoLanes &&
    wipLanes !== nextLanes &&
